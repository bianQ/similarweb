...
def create_spider(use_plugin, use_proxy, window):
    spider = Spider(use_plugin=use_plugin, use_proxy=use_proxy, window=window)
    spider.run()
...
def run(use_plugin, use_proxy, workers, times, window):
    pool = MultiThread(max_workers=workers)
    for _ in range(times):
        pool.execute(create_spider, use_plugin, use_proxy, window)
